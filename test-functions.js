...
async function testFunctionSyntaxOnly(functionName) {
  try {
    console.log(`\n🔍 Verificando sintaxis: ${functionName}`);
    ...
    if (functionModule.handler && typeof functionModule.handler === 'function') {
      console.log(`✅ ${functionName} - sintaxis correcta`);
      return true;
    } else {
      console.log(`❌ ${functionName} - handler no válido`);
      return false;
    }
    
  } catch (error) {
    console.log(`❌ ${functionName} - error de sintaxis: ${error.message}`);
    return false;
  }
}
